        ...
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && (
              <p>Sorry Currently all Mugs are out stock</p>
            )}
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.tittle in Mugs) {
      if (
        !Mugs[item.tittle].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        Mugs[item.tittle].color.push(item.color);
      }
      if (
        !Mugs[item.tittle].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        Mugs[item.tittle].size.push(item.size);
      }
    } else {
      Mugs[item.tittle] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        Mugs[item.tittle].color = [item.color];
        Mugs[item.tittle].size = [item.size];
      }
    }
  }
  //console.log(JSON.parse(JSON.stringify(Mugs)));
